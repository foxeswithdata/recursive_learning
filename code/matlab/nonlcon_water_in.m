...
function [c, ceq] = nonlcon_water_in(X, param)
...
for i = 2:size(X,2)
    ...
    xw = X(4,i-1) + param.deltat * (param.ri(i) - param.kw * kp * X(2, i-1));
...
xw = param.X0(3) + param.deltat * (param.ri(i) - param.kw * kp * param.X0(1));
